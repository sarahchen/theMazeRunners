...
Character::Character(int xpos, int ypos, carType car, GLuint Id)
{
	...
	textID = Id;
	gameOver = false;
	//livesRemaining = STARTLIVES;  //future expansion
	score = 0;
	...
	//set initial parameters and configuration
	headingAngle = M_PI_2;
	initPos.x = xpos;
	initPos.y = ypos;
...
bool Character::isValidMove()
{
	...
	// if makes it through all that then this is a valid move!
	else { return true; }
}
...
//future function to expand into multiple lives per game
...
void Character::draw()
{
	...
	//draw car based on which one this character is
	if (thisCar == regCar) {
